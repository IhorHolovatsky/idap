...
const stylesBlob = "src/scss/**/*.scss";
...
gulp.task("copyJsFiles", function() {
  return gulp.src(jsFiles).pipe(gulp.dest(distDirectory));
});
...
gulp.task("processFonts", function() {
  return gulp.src(fontsBlob).pipe(gulp.dest(`${distDirectory}/fonts/`));
});
